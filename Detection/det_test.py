...
type45 = type45.split(',')
type19={'pl30': 382, 'pl5': 324, 'w57': 201, 'io': 602, 'p5': 277, 'ip': 254, 'i2': 281, 'i5': 1463, 'i4': 530, 'pl80': 568, 'p11': 1204, 'pl60': 589, 'p26': 298, 'pl40': 1046, 'p23': 200, 'pl50': 851, 'pn': 2341, 'pne': 1852, 'po': 655}
type19 = list(type19.keys())
...
def load_mask(annos, datadir, imgid, imgdata):
    ...
    for obj in img['objects']:
        ...
        if obj.get('polygon', False) and len(obj['polygon'])>0:
...
def draw_all(annos, datadir, imgid, imgdata, color=(0,1,0), have_mask=True, have_label=True):
    ...
    for obj in img['objects']:
        ...
        ss = obj['category']
        if obj.get('correct_catelog', False):
...
def get_refine_rects(annos, raw_rects, minscore=0.02):
    cover_th = 0.5
    refine_rects = {}
...
def eval_annos(annos_gd, annos_rt, iou=0.75, imgids=None, check_type=True, types=None, minscore=0.04, minboxsize=0, maxboxsize=400, match_same=True):
    ac_n, ac_c = 0,0
    rc_n, rc_c = 0,0
    if imgids==None:
    ...
    for imgid in imgids:
        ...
        if types!=None:
            for i in range(len(match_g)):
                if not types.get(objs_g[i]['category'], False):
                    match_g[i] = -2
            for i in range(len(match_r)):
                if not types.get(objs_r[i]['category'], False):
                    match_r[i] = -2
        for i in range(len(match_r)):
            if objs_r[i].get('score', False) and objs_r[i]['score']<minscore:
                match_r[i] = -2
        matches = []
        ...
        for i in range(len(match_g)):
            ...
            if not (boxsize>=minboxsize and boxsize<maxboxsize):
                erase = True
            #if types!=None and not types.get(objs_g[i]['category'], False):
            #    erase = True
            if erase:
                if match_g[i] >= 0:
                    match_r[match_g[i]] = -2
...
def parse_args():
    parser = argparse.ArgumentParser(description='Test script for tracffic sign detecction')
...
if __name__ == '__main__':
    args = parse_args()
    annos = json.loads(open(args.anno_file).read())
    results = json.loads(open(args.result_file).read())

    sm = eval_annos(annos, results, iou=0.5, types=type19, check_type=True)
    print(sm['report'])
