...
def main():
    ...
    print(dataset_size)
    train_ds, val_ds = torch.utils.data.random_split(ds, [10000, 4463])
    ...
    test_model.load_state_dict(torch.load('params.pkl'))
    optimizer = optim.Adam(test_model.parameters(), lr = 0.005)
    result = train(test_model, 5, optimizer, train_loader)
